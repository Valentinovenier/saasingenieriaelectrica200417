...
export const onRequest: PagesFunction = async (context) => {
  ...
  const db = env.DB;
  const SECRET = env.JWT_SECRET as string; // Asegúrate de definir esto en tus secretos
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Error en el servidor: " + e.message }), { status: 500 });
  }
};
